...
class TestFilterWorker:
    ...
    async def test_process_job_missing_filter_field(self):
        ...
        job = Job(
            id="test-001",
            prompt="Process task without category",
            task="do something"
        )
    ...
    async def test_process_job_case_insensitive_matching(self):
        ...
        for priority_value, should_match in test_cases:
            ...
            if should_match:
                assert result.startswith("ACCEPTED:"), f"Failed for priority='{priority_value}'"
            else:
                assert result.startswith("REJECTED:"), f"Failed for priority='{priority_value}'"
