    ...
    try:
        ...
        if stats.stats_source == "management_api":
            ...
            if stats.message_count_ready is not None:
                table.add_row("├─ Ready (awaiting processing)", str(stats.message_count_ready))
            if stats.message_count_unacknowledged is not None:
                table.add_row("└─ Unacknowledged (processing)", str(stats.message_count_unacknowledged))
            
            # Byte information
            if stats.message_bytes is not None:
                table.add_row("Total Bytes", f"{stats.message_bytes:,} bytes ({stats.message_bytes / 1024 / 1024:.1f} MB)")
            ...
            if stats.message_bytes_unacknowledged is not None:
                table.add_row("└─ Unacked Bytes", f"{stats.message_bytes_unacknowledged:,} bytes")
            ...
            if stats.stats_source == "amqp_fallback":
                ...
                table.add_row("Stats Source", "AMQP fallback - enable management plugin for details")
            else:
                table.add_row("Messages in Queue", "Unknown (queue may not exist)")
                table.add_row("Active Consumers", "Unknown (queue may not exist)")
                table.add_row("Stats Source", "Unavailable - check queue name and connection")
        ...
        if stats.consumer_count is not None and stats.consumer_count == 0:
            console.print(Panel("[yellow]⚠️  No active consumers - jobs will not be processed[/yellow]", title="Warning"))
        
        if stats.message_count is not None and stats.message_count > 1000:
            console.print(Panel(f"[yellow]⚠️  High message backlog: {stats.message_count} messages[/yellow]", title="Warning"))
        
        # Show info about stats source
        if stats.stats_source == "amqp_fallback":
            console.print(Panel("[blue]💡 Enable RabbitMQ management plugin for detailed statistics[/blue]", title="Info"))
        elif stats.stats_source == "unavailable":
            console.print(Panel("[red]❌ Could not retrieve queue statistics - check queue name and connection[/red]", title="Error"))
...
def check_health(queue_name: str):
    ...
    try:
        with console.status(f"Checking health for queue '{queue_name}'..."):
            is_healthy, issues, stats = asyncio.run(check_queue_health_async(queue_name))
        ...
        if stats:
            console.print(f"[dim]Messages: {stats.message_count}, Consumers: {stats.consumer_count}[/dim]")
...
def show_errors(queue_name: str, limit: int):
    ...
    try:
        ...
        if not failed_messages:
            console.print(f"[green]No failed messages found in queue '{queue_name}'[/green]")
            return
        ...
        if len(failed_messages) >= limit:
            console.print(f"[dim]Showing first {limit} errors. Use --limit to see more.[/dim]")
...
def show_connection_status():
    ...
    try:
        ...
        if is_connected:
            ...
            console.print(f"[dim]URL: {config.rabbitmq_url}[/dim]")
            console.print("[yellow]💡 Make sure RabbitMQ is running and accessible[/yellow]")
