...
class BrokerManager:
        ...
        for attempt in range(max_retries):
            try:
                self.connection = await aio_pika.connect_robust(
                    ...
                    client_properties={"application": "llmq"}
                )
                self.channel = await self.connection.channel()
                await self.channel.set_qos(prefetch_count=self.config.vllm_queue_prefetch)
    ...
    async def setup_queue_infrastructure(self, queue_name: str) -> tuple[AbstractQueue, AbstractExchange, AbstractQueue]:
        """
    ...
    async def publish_result(self, results_exchange: AbstractExchange, result: Result) -> None:
    ...
    async def consume_results(self, queue_name: str, callback: Callable) -> AbstractQueue:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    return QueueStats(
                        ...
                        message_count_unacknowledged=data.get('messages_unacknowledged'),
                        consumer_count=data.get('consumers'),
                        message_bytes=data.get('message_bytes'),
                        message_bytes_ready=data.get('message_bytes_ready'),
                        message_bytes_unacknowledged=data.get('message_bytes_unacknowledged'),
    ...
    async def get_failed_messages(self, queue_name: str, limit: int = 100) -> list[dict]:
        ...
        try:
            failed_queue = await self.channel.declare_queue(failed_queue_name, passive=True)
